function [X_proj_new,Efinal] = frankWolfeSolverRape(A,B,params)
...
for ii = 1:numel(shifts)
    params.translation = shifts(ii);
    params.step = shifts(2)-shifts(1);
    [X_proj_new{ii},Efinal(ii)] = frankWolfeSolver(A,B,params);
end

figure,plot(Efinal)
[Efinal,idx] = min(Efinal);
X_proj_new = X_proj_new{idx};
